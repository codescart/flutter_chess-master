...
import 'package:chess_bot/MainGameModel/FreePlay/chess_board/flutter_chess_board.dart';
import 'package:chess_bot/MainGameModel/FreePlay/chess_board/src/chess_sub.dart';
import 'package:chess_bot/MainGameModel/FreePlay/eval/ai.dart';
...
class ChessControllerd {
  ...
  void onMove(move) {
    ...
    ChessControllerd.moveFrom = move['from'];
    ChessControllerd.moveTo = move['to'];
    //print the move
    print('onMove: $move');
    ...
    if (kIsWeb) {
      ...
      Future.delayed(Duration(milliseconds: 100)).then((value) {
        ChessAI.entryPointMoveFinderNoIsolateAsync(
                game.fen, (prefsd.getInt('set_depth') ?? 0))
  ...
  bool makeBotMoveIfRequired() {
    ...
    //make move if needed
    if (((game?.game?.turn ?? Color.flip(botColor)) == Colors.black) &&
        prefsd.getBool('botd')) {
      findMove();
      return true;
    }
  ...
  void onDraw() {
    ...
    showAnimatedDialog(
        title: stringsd.draw,
        text: stringsd.draw_desc,
        onDoneText: stringsd.replay,
        onDone: (value) {
          game.reset();
          update();
        });
  ...
  void onCheckMate(color) {
    ...
    var winner = color == PieceColor.White ? stringsd.black : stringsd.white;
    var loser = color == PieceColor.White ? stringsd.white : stringsd.black;
    //show the dialog
    showAnimatedDialog(
        title: stringsd.checkmate,
        text: stringsd.check_mate_desc(loser, winner),
        onDoneText: stringsd.replay,
        onDone: (value) {
          game.reset();
          update();
  ...
  void resetBoard() {
    showAnimatedDialog(
        title: stringsd.replay,
        text: stringsd.replay_desc,
  ...
  void _undo() {
    game.undo() != null
        ? controller.refreshBoard()
        : showAnimatedDialog(
            title: stringsd.undo, text: stringsd.undo_impossible);
  }

  void switchColors() {
    whiteSideTowardsUser = !whiteSideTowardsUser;
    prefsd.setBool('whiteSideTowardsUserd', whiteSideTowardsUser);
    update();
  }
  ...
  void changeBoardStyle() async {
    ...
    await showGeneralDialog<String>(
      ...
      transitionBuilder: (context, a1, a2, widget) {
        ...
        return Transform(
            ...
            child: Opacity(
                opacity: a1.value,
                child: AlertDialog(
                  title: Text(stringsd.choose_style),
                  content: Column(
      ...
      //save the value to the prefsd
      prefsd
          .setString("board_styled", value ?? prefsd.getString("board_styled"))
          .then((value) => update());
  ...
  void onSetDepth() {
    List difficulties = stringsd.difficulties.split(',');
    BuildContext ctx;

    showAnimatedDialog(
        title: stringsd.difficulty,
        ...
        children: [
          RadioGroup.builder(
              ...
              onChanged: (value) {
                ...
                //save in the prefsd
                prefsd.setInt('set_depthd', diff);
                //then pop the nav
                Navigator.of(ctx).pop();
              },
              groupValue: difficulties[prefsd.getInt('set_depthd') ?? 0],
              items: difficulties,
  ...
  void _onFenMobile() {
    List difficulties = stringsd.fen_options.split(',');
    BuildContext ctx;

    var fen = game.fen;

    showAnimatedDialog(
        title: stringsd.copy_fen,
        setStateCallback: (ctx0, setState) {
          ctx = ctx0;
        },
        onDone: (value) {
